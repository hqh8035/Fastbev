...
from scripts.inference import postprocess_predictions
...
def run_onnx_inference(onnx_model_path, image_path, camera_intrinsic, 
                       det3d_threshold, seg_thres_list, quant=False,
    ...
    sess = HB_ONNXRuntime(onnx_model_path)

    # 预处理
    image_tensor, camera_intrinsic = preprocess(image_path, camera_intrinsic, quant=quant, onnx_infer=onnx_infer)
...
def infer_image_list(quant, onnx_model, image_list, camera_intrinsic, 
                     det3d_thres, seg_thres_list,
                     fps, frame_size, save_dir='onnx_infer_results', 
                     video_name='onnx_infer_video.mp4', make_video=False,
                     onnx_infer=False):
    ...
    if make_video:
        result_image_list = glob.glob(os.path.join(args.save_dir, '*.jpg'))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--onnx_model_path", type=str,default='onnx/fastbev_model.onnx',
                        help="Path to the ONNX model file")
    parser.add_argument("--image_dir", type=str, default='infer_images',)
    parser.add_argument("--image_path", type=str,default='debug_infer_images/0.jpg',
                        help="Path to the input image")
    parser.add_argument("--save_dir", type=str, default="onnx_infer_results_horizon",
    ...
    parser.add_argument("--video_file", type=str, default="infer_video.mp4", help="Output video file path")
    parser.add_argument("--fps", type=int, default=20, help="Video FPS")
    parser.add_argument("--frame_size", type=int, nargs=2, default=[960, 816], help="Video frame size (w h)")
    ...
    infer_image_list(args.quant, args.onnx_model_path, exist_image_list, 
                     np.array(args.camera_intrinsic).reshape(3, 3), 
                     args.det3d_threshold, args.seg_thres_list,
                     args.fps, args.frame_size,
                     save_dir=args.save_dir, make_video=args.make_video,
                     onnx_infer=args.onnx_infer)