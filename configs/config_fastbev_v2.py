...
# FastBEV特定配置
_C.MODEL.FASTBEV = CN()
_C.MODEL.FASTBEV.WITH_SEG = False                       # 不要seg的head
...
_C.MODEL.FASTBEV.DET_OUT_CHANNELS = 27                        # 检测输出的channels数, center + obj_height + offset + lwh + yaw = 1 + 1 + 2 + 3 + 2 
...
_C.DATASET.TRAIN.nuscenes.generation_gt = 'standard'        # 用于生成gt, 和FASTBEV.generate_det_gt_format保持一致           # 'gaussian' for center map gaussian/ 'standard' for standard focal loss
_C.DATASET.TRAIN.nuscenes.annotations_file = 'all_labels/nuscenes_bbox3d_labels/all_train_bbox_labels.txt'
...
_C.DATASET.VAL.nuscenes.generation_gt = 'standard' 
_C.DATASET.VAL.nuscenes.annotations_file = 'mini_data/debug_mini_labels.txt'
